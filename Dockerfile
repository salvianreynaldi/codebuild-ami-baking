FROM ubuntu:bionic
RUN DEBIAN_FRONTEND=noninteractive apt update && apt install -y --no-install-recommends software-properties-common python-pip git unzip
RUN pip install --upgrade ansible==2.7.10 pip boto3 botocore awscli
